...
ind_ratio <- rr_n %>% 
  ...
  group_by(district) %>% 
  summarise(ratio = median(round(ratio, 0)),
            ratio_max = max(round(ratio, 0)),
            ratio_min = min(round(ratio, 0)),
            warning = case_when(ratio_max > ratio + 2 |
                                  ratio_min < ratio - 2 ~ "warning")
            ) %>% pull(ratio)
...
ggplot(aes(x=academicyear, y=rank)) +
  ...
  labs(title = glue::glue("Ratio of students per teacher by racial/ethnic group in {unique(rr_viz$district)}"),
       ...
       caption = glue::glue("Reference ratio of students to teachers is approximately **{ind_ratio} : 1**, irrespective of race/ethnicity"))
...
## Representation by distict
```{r, heat map of districts single year}
#Heat Map
```
